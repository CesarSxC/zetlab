...
import { X, Search, Loader2, User, Mail, Phone, Calendar, IdCard, Hospital, Percent } from 'lucide-react'
...
export const ModalBM: React.FC<ModalProps> = ({ isOpen, onClose, onPersonSelect }) => {
  ...
  const fetchPersonas = async () => {
    setIsLoading(true);
    try {
    const respuesta = await apiCall<Medicos>('get_medico')
  ...
  const handleSearch = (term: string) => {
    ...
    const filtered = personas.filter((persona) => {
      ...
      const espec = persona.nombre_esp.toLowerCase();
      const cmp = persona.codigo_cmp.toLowerCase();
      return fullName.includes(term.toLowerCase()) || document.includes(term.toLowerCase())
      || espec.includes(term.toLowerCase()) || cmp.includes(term.toLowerCase()) ;
    });
    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          >
            <div className="relative bg-gradient-to-r from-cyan-400 to-sky-400 p-6">
              <h2 className="text-2xl font-bold text-white mb-2">Buscar Médicos</h2>
              <button
                onClick={onClose}
            ...
            <div className="p-6 space-y-6">
              <div className="relative">
                <Input
                  placeholder="Buscar por nombres, especialidad y DNI..."
                  value={searchTerm}
              ...
              {isLoading ? (
                ...
                <ul className="space-y-4 max-h-[400px] overflow-y-auto pr-2">
                  {filteredPersonas.map((persona) => (
                    <li key={persona.id_medico} className="bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow">
                      <div className="p-4">
                        <div className="flex items-center justify-between mb-2">
                          <h3 className="text-lg font-semibold text-gray-800 flex items-center">
                            <User className="mr-2 text-blue-500" size={20} />
                            {persona.id_medico} - {persona.apellido_p}, {persona.nombre_p}
                          </h3>
                          <span className="text-sm uppercase text-gray-600 font-semibold">{persona.nombre_esp}</span>                         
                        </div>
                        <div className="grid grid-cols-2 gap-2 text-sm">
                          ...
                          <p className="flex items-center text-gray-600">
                            <IdCard className="mr-2 text-blue-400" size={18} />
                            {persona.nombre_tdoc}: {persona.num_doc}
                          </p>
                          <p className="flex items-center text-gray-600">
                            <Hospital className="mr-2 text-blue-400" size={18} />
                            CMP: {persona.codigo_cmp}
                          </p>
                          <p className="flex items-center text-gray-600">
                            <Percent className="mr-2 text-blue-400" size={18} />
                            Comisión: {persona.comision}
                          </p>
                        </div>
                      </div>
