...
interface ModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAnalisisSelect: (person: any) => void;
}
...
interface Analisis {
  ...
  export const ModalBA: React.FC<ModalProps> = ({ isOpen, onClose, onAnalisisSelect }) => {
    ...
    const handleSearch = (term: string) => {
      ...
      const filtered = analisis.filter((analisis) =>
        analisis.nombre_analisis.toLowerCase().includes(term.toLowerCase()) ||
        analisis.nombre_categoria_ana.toLowerCase().includes(term.toLowerCase())
      );
      setFilteredAnalisis(filtered);
    };
  
    const toggleSelectAnalisis = (analisis: Analisis) => {
      setSelectedAnalisis((prev) => {
        if (prev.some((a) => a.id_analisis === analisis.id_analisis)) {
          return prev.filter((a) => a.id_analisis !== analisis.id_analisis); 
        }
        return [...prev, analisis];
      });
    };
  
    const handleConfirmSelection = () => {
      onAnalisisSelect(selectedAnalisis.flat());
      onClose();
    };
            ...
            >
              ...
              <div className="p-6 space-y-6">
                ...
                {isLoading ? (
                  ...
                  <ul className="space-y-4 max-h-[400px] overflow-y-auto pr-2">
                    {filteredAnalisis.map((analisisl) => (
                      <li
                        key={analisisl.id_analisis}
                        className={`bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow ${
                          selectedAnalisis.some((a) => a.id_analisis === analisisl.id_analisis) && "bg-blue-50"
                        }`}
                      >
                        <div className="p-4">
                          ...
                          <div className="grid grid-cols-2 gap-2 text-sm">
                            ...
                            <p className="flex items-center text-gray-600">
                              <PencilRuler className="mr-2 text-blue-400" size={16} />
                              Rango Referencial: {analisisl.rango_referencial}
                            </p>
                          </div>
                        </div>
                        <div className="bg-gray-50 px-4 py-3 border-t border-gray-200 flex justify-end">
                          <Button
                            variant="outline"
                            onClick={() => toggleSelectAnalisis(analisisl)}
                            className={`${
                              selectedAnalisis.some((a) => a.id_analisis === analisisl.id_analisis)
                ...
                <div className="flex justify-end gap-2">
                  <Button onClick={onClose} variant="outline" className="text-red-600 hover:text-red-700">
                    Cancelar
                  </Button>
                  <Button onClick={handleConfirmSelection} className="bg-blue-600 text-white hover:bg-blue-700">
                    Confirmar Selección
                  </Button>
                </div>
              </div>
