...
export const ModalEDES: React.FC<ModalProps> = ({ isOpen, onClose, idEspecialidad, initialName, onSucces }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiCall("update_especialidad", {
        ...
        >
          ...
          >
            <div className="relative bg-gradient-to-r from-yellow-500 to-yellow-300 p-6 pb-14">
              <h2 className="text-3xl font-bold text-white mb-2">Actualizar Especialidad</h2>
              <p className="text-yellow-100">Modifica los detalles para actualizar la especialidad</p>
              <button
                onClick={onClose}
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6 -mt-8">
              <div className="bg-gray-50 rounded-xl p-4 shadow-lg space-y-4">
                <div className="space-y-2">
                  <Label htmlFor="name" className="text-sm font-medium text-gray-700">Nombre Especialidad</Label>
                  <div className="relative">
                    <Input
                      ...
                      required
                    />
                    <BriefcaseMedical className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400" size={18} />
                  </div>
                </div>
              </div>
